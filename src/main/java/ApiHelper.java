...
public class ApiHelper {
        ...
        RequestBody body = new MultipartBody.Builder().setType(MultipartBody.FORM)
                ...
                .addFormDataPart("file","Web_Photo_Editor (1).jpg",
                        RequestBody.create(MediaType.parse("application/octet-stream"),
                                new File("/Users/mnats/Documents/Screenshot_20210527-083757_PicsArt.jpg")))
    ...
    public static void removeUser(String userKey) throws IOException {
        OkHttpClient client = new OkHttpClient().newBuilder()
                .build();
        MediaType mediaType = MediaType.parse("application/json");
        RequestBody body = RequestBody.create(mediaType, "{\n\t\"cover\": \"streams/show/cover.json\",\n\t\"photo\": \"\"\n}");
        Request request = new Request.Builder()
                .url("https://api.picsart.com/users/remove?key="+userKey)
                .method("POST", body)
                .addHeader("Content-Type", "application/json")
                .build();
        Response response = client.newCall(request).execute();
        System.out.println("User by key " + userKey + " successfully deleted");
    }
